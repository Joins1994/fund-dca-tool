    ...
    data.forEach(item => {
        ...
        card.innerHTML = `
            ...
            <div style="margin-top: 12px; padding: 10px; background: #f8fafc; border-radius: 8px; font-size: 0.85rem;">
                <strong>💡 适合人群:</strong> ${config.bestFor || '综合配置型投资者'}
                ${config.fundOptions && config.fundOptions.length > 0 ? `<br><strong>📈 推荐基金:</strong> ${config.fundOptions[0]}` : ''}
            </div>
        `;
...
async function renderRecords() {
    ...
    try {
        ...
        records.forEach(record => {
            const buyPoint = record.buy_index_point > 0 ? record.buy_index_point.toFixed(2) : '--';
            html += `
                <div class="record-item">
                    <div>
                        <div style="font-weight: 600;">${record.index_name}</div>
                        <div style="font-size: 0.85rem; color: var(--text-light);">${record.invest_date} · 买入点位: ${buyPoint}</div>
                    </div>
...
async function exportRecords() {
    try {
        ...
        const records = result.records;
        let csv = '\uFEFF日期,指数,买入点位,金额\n';
        records.forEach(r => {
            const buyPoint = r.buy_index_point > 0 ? r.buy_index_point.toFixed(2) : '';
            csv += `${r.invest_date},${r.index_name},${buyPoint},${r.amount}\n`;
        });
